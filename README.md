...
**`policy.enabled`** drives a **separate** ACM **Policy** / **Placement** / **PlacementBinding** pipeline. Hub **Policy** status (for example empty compliance when placement selects zero clusters) does **not** indicate whether ManifestWork already delivered the bundle—use **ManifestWork** status and spoke **ConfigMap**/**Proxy** to confirm rollout.
