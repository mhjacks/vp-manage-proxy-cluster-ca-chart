...
Target namespaces must receive the labeled Bundle (label with **`cluster-ca.vp.io/trust-bundle-target: "true"`**). Example:
...
| trustTest.route.name | string | `"config-demo"` | Default ingress route for apps.<clusterDomain> pattern when not set per namespace. |
| trustTest.route.namespace | string | `"config-demo"` |  |
| trustTest.schedule | string | `"0 */6 * * *"` |  |
| trustTest.successfulJobsHistoryLimit | int | `3` |  |
| trustTest.suspend | bool | `false` |  |
| trustTest.targets | list | `[]` | Optional explicit targets merged with ACM/local discovery. |
